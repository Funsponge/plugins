...
The **wpp_get_views** template tag retrieves the views count of a single post. It accepts two parameters: the *post ID* (eg. echo wpp_get_views(15)) and *time range* (eg. echo wpp_get_views(15, 'weekly')). Only the *post ID* is required. If *time range* isn't provided the function will retrieve the total amount of views, otherwise it'll return the number of views received within the selected time range. If the post has no views the function will return false instead.
...
Updates will come depending on my work projects (I'm a full-time web developer) and the amount of time I have on my hands.
...
Chances are that your question has been already answered either in the [Support Forum](http://wordpress.org/support/plugin/wordpress-popular-posts) or here in the FAQ section, so I will simply ignore your thread. Or maybe I just haven't read your post yet so please be patient (in the meanwhile, search the [Support Forum](http://wordpress.org/support/plugin/wordpress-popular-posts) for an answer).
